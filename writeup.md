...
My Part A implementation uses dynamic task assignment for all parallel systems.
...
For Part B, I implemented dependency tracking only in
...
For Part B's synchronous `run()`, I kept a separate fast path modeled after the
...
I implemented several synchronous custom tests:
...
The main test I would highlight is `repeated_launches_sync`.
...
I ran the custom test on the AWS ARM instance using the assignment test
...
For Part B, I added `async_returns_before_completion`. This test is specific to
